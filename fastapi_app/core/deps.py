...
from ..services.cache_service import (
    async_get_cached_auth_token_version,
    async_get_cached_user_profile,
    async_set_cached_auth_token_version,
    async_set_cached_user_profile,
...
async def _load_user_from_profile_cache_async(user_id: int, auth_version: int) -> User | None:
    local_profile = get_local_cached_user_profile(user_id)
    if local_profile:
        return _build_user_from_cached_profile(local_profile, auth_version=auth_version)

    cached_profile = await async_get_cached_user_profile(user_id)
    if cached_profile:
        set_local_cached_user_profile_payload(cached_profile)
        return _build_user_from_cached_profile(cached_profile, auth_version=auth_version)
    return None
...
async def _load_current_user_async(token: str, db: AsyncSession, *, use_cache: bool) -> User:
    user_id, auth_version = _decode_identity(token)
    if use_cache:
        cached_auth_version = await async_get_cached_auth_token_version(user_id)
        if cached_auth_version is not None and cached_auth_version != auth_version:
            raise HTTPException(status_code=401, detail="Not authenticated")
        if cached_auth_version is not None:
            cached_user = await _load_user_from_profile_cache_async(user_id, auth_version)
            if cached_user is not None:
                return cached_user
        lock_key = _auth_rebuild_lock_key(user_id)
        lock_value = build_lock_value()
        if await async_acquire_lock(lock_key, lock_value):
            try:
                cached_auth_version = await async_get_cached_auth_token_version(user_id)
                if cached_auth_version is not None and cached_auth_version != auth_version:
                    raise HTTPException(status_code=401, detail="Not authenticated")
                ...
                if not user or user.auth_token_version != auth_version:
                    raise HTTPException(status_code=401, detail="Not authenticated")
                await async_set_cached_auth_token_version(user.id, user.auth_token_version)
                await async_set_cached_user_profile(user)
                return user
            finally:
                await async_release_lock(lock_key, lock_value)
        else:
            for _ in range(AUTH_REBUILD_MAX_RETRIES):
                await asyncio.sleep(AUTH_REBUILD_WAIT_SECONDS)
                cached_auth_version = await async_get_cached_auth_token_version(user_id)
                if cached_auth_version is None:
                    continue
                if cached_auth_version != auth_version:
...
async def get_current_user_optional_async(
    ...
    if not authorization:
        return None
    token = _extract_bearer_token(authorization)
    return await _load_current_user_async(token, db, use_cache=True)
